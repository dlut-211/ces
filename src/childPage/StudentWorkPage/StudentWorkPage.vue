<template>
  <div class="StudentWorkPage">
    <!-- 获取章节作业 -->
    <Modal v-model="ShowWorkList" title="章节作业" width="800px" :mask-closable="false"
    ...
    <!-- 上传作业 -->
    <Modal v-model="SubmitWork" title="上传作业（仅支持上传:war、zip、rar类型的文件）" width="800px" :mask-closable="false"
           style="position:fixed;z-index:99999">
      ...
      <div slot="footer">
        ...
        <Button v-if="!SubmitWorkForm.WorkPath" type="primary" disabled size="large" @click="SubmitWorkAction()">确定
        </Button>
      </div>
    </Modal>
    <tableModule :object="tableModule" @changePage="changePage" @changeSize="changeSize"
                 @addStudentWork="addStudentWork=true;$refs['addStudentWorkForm'].resetFields();"></tableModule>
    <Modal v-model="huanbi" title="课程环比图" width="800px" :mask-closable="false" style="position:fixed;z-index:99999">
      <div id="echarts1" :style="{width: '750px', height: '500px'}"></div>
      <div slot="footer"></div>
    </Modal>
    <Modal v-model="leida" title="雷达图" width="800px" :mask-closable="false" style="position:fixed;z-index:99999">
      <div id="echarts2" :style="{width: '1000px', height: '500px'}"></div>
      <div slot="footer"></div>
    </Modal>
    <Modal v-model="bingtu" title="饼图" width="800px" :mask-closable="false" style="position:fixed;z-index:99999">
...
<script>
  ...
  return str.replace(/<[^>]+>/g,"");//去掉所有的html标记
            ...
            SubmitWorkAction: function () {
                ...
                const laytime = new Date(this.layouttime).getTime();
            ...
            getWorkByChapter: function () {
                const params = this.findWorkList;
            ...
            editStudentWorkHandleSubmit: async function (name) {
                const result = await this.$refs.editStudentWorkForm.validate(valid => {
                });
                if (result) {
            ...
            // 查询方法
            find: function (a) {
                this.findStudentWorkForm.ClassRoomId = a.ClassRoomId ? a.ClassRoomId : "2";
                this.getchapterWorkList();
            },
            // 改变页码
